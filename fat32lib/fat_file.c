	...
	if (handle->access_flags & FAT_FILE_ACCESS_WRITE)
	{
		...
		if (handle->directory_entry.raw.ENTRY.STD.first_cluster_lo == 0 && handle->directory_entry.raw.ENTRY.STD.first_cluster_hi == 0)
		{
			...
			*/
			...
			{
				uint32_t page_size = volume->device->get_page_size(volume->device->Media);
			...
			*/		
			ret = volume->device->ReadSector(volume->device->Media, file_entry.sector_addr, buffer);
			...
			*/			
			ret = volume->device->WriteSector(volume->device->Media, file_entry.sector_addr, buffer);
			if (ret != STORAGE_SUCCESS)
			{
				handle->magic = 0;
				#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
		...
		*/
		if (entry->raw.ENTRY.STD.first_cluster_lo != 0x0 || entry->raw.ENTRY.STD.first_cluster_hi != 0x0) 
		{
			...
			*/
			...
			// read the sector that contains the entry
			*/		
			ret = volume->device->ReadSector(volume->device->Media, entry->sector_addr, buffer);
			if (ret != STORAGE_SUCCESS)
			{
				handle->magic = 0;
				#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			...
			*/
			...
			// write the modified entry to the media
			*/			
			ret = volume->device->WriteSector(volume->device->Media, entry->sector_addr, buffer);
			if (ret != STORAGE_SUCCESS)
			{
				handle->magic = 0;
				#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
...
uint16_t fat_file_set_buffer(FAT_FILE* file, unsigned char* buffer)
{
	if (file->buffer_head != file->buffer)
	{
		...
		ret = file->volume->device->ReadSector(file->volume->device->Media, sector_address, file->buffer);
		if (ret != STORAGE_SUCCESS)
		{
			file->busy = 0;
			return FAT_CANNOT_READ_MEDIA;
	...
	while (*query.current_entry.raw.ENTRY.STD.name != 0)
	{
		if (query.current_entry.raw.ENTRY.LFN.lfn_checksum == checksum)
		{
			...
			*/
			...
			ret = volume->device->ReadSector(volume->device->Media, query.current_entry.sector_addr, buffer);
			if (ret != STORAGE_SUCCESS)
				return ret;

			memcpy(buffer + query.current_entry.sector_offset, &query.current_entry.raw, sizeof(entry.raw));

			ret = volume->device->WriteSector(volume->device->Media, query.current_entry.sector_addr, buffer);
			if (ret != STORAGE_SUCCESS)
				return ret;
		}
		/*
...
uint16_t fat_file_rename(FAT_VOLUME* volume, char* original_filename, char* new_filename)
{
	...
	*/
	...
	if (*original_entry.name != 0)
	{
		...
		*/
		FAT_SET_LOADED_SECTOR(volume, FAT_UNKNOWN_SECTOR);
		ret = volume->device->ReadSector(volume->device->Media, new_entry.sector_addr, buffer);
		if (ret != STORAGE_SUCCESS)
		{
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			LEAVE_CRITICAL_SECTION(file->volume->sector_buffer_lock);
		...
		ret = volume->device->WriteSector(volume->device->Media, new_entry.sector_addr, buffer);
		if (ret != STORAGE_SUCCESS)
		{
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			LEAVE_CRITICAL_SECTION(file->volume->sector_buffer_lock);
		...
		*original_entry.raw.ENTRY.STD.name = FAT_DELETED_ENTRY;
		ret = volume->device->ReadSector(volume->device->Media, original_entry.sector_addr, buffer);
		if (ret != STORAGE_SUCCESS)
		{
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			LEAVE_CRITICAL_SECTION(file->volume->sector_buffer_lock);
		...
		ret = volume->device->WriteSector(volume->device->Media, original_entry.sector_addr, buffer);
		if (ret != STORAGE_SUCCESS)
		{
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			LEAVE_CRITICAL_SECTION(file->volume->sector_buffer_lock);
...
uint16_t fat_file_alloc(FAT_FILE* file, uint32_t bytes)
{
	...
	*/
	#if defined(FAT_OPTIMIZE_FOR_FLASH)
	{
		if (file->access_flags & FAT_FILE_FLAG_OPTIMIZE_FOR_FLASH)
		{
			...
			page_size = file->volume->device->get_page_size(file->volume->device->Media);
			...
			*/
			...
			while (!fat_is_eof_entry(file->volume, current_cluster))
			{
				...
				*/
				file->volume->device->erase_sectors(file->volume->device->Media, start_address, end_address - 1);
			}
		}
		else
		{
	...
	*/				
	if (!file->directory_entry.raw.ENTRY.STD.first_cluster_lo && !file->directory_entry.raw.ENTRY.STD.first_cluster_hi) 
	{
		...
		*/
		ret = file->volume->device->ReadSector(file->volume->device->Media, file->directory_entry.sector_addr, buffer);
		if (ret != STORAGE_SUCCESS) 
		{
			file->busy = 0;
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
		...
		*/
		...
		// write the modified entry to the media
		*/			
		ret = file->volume->device->WriteSector(file->volume->device->Media, file->directory_entry.sector_addr, buffer);
		if (ret != STORAGE_SUCCESS) 
		{
			file->busy = 0;
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
					...
					if (handle->no_of_clusters_after_pos == 0)
					{
						...
						*/
						ret = fat_file_alloc(handle, handle->volume->no_of_bytes_per_serctor * 
								handle->volume->device->get_page_size(handle->volume->device->Media));
						if (ret != STORAGE_SUCCESS) 
						{
							/*
							// set result code
		...
		*/
		if (handle->buffer_head == handle->op_state.end_of_buffer) 
		{
			...
			*/
			...
			{
				ret = handle->volume->device->WriteSector(
					handle->volume->device->Media, handle->op_state.sector_addr, handle->buffer);
			}
			else 
			{
				/*
				...
				*/
				...
				// storage device
				*/
				ret = handle->volume->device->WriteSectorAsync(
					handle->volume->device->Media, 
					handle->op_state.sector_addr, 
					handle->buffer, 
					&handle->op_state.storage_state,
	...
	*/
	if (handle->buffer_dirty) 
	{
		if (!handle->op_state.async_state)
		{
			...
			*/			
			ret = handle->volume->device->ReadSector(
				handle->volume->device->Media, handle->op_state.sector_addr, handle->buffer);
		}
		else
		{
			...
			*/
			ret = handle->volume->device->ReadSectorAsync(
				handle->volume->device->Media, handle->op_state.sector_addr, handle->buffer,
	...
	*/
	if (handle->access_flags & FAT_FILE_ACCESS_WRITE)
	{
		...
		if (!(handle->access_flags & FAT_FILE_FLAG_NO_BUFFERING))
		{
			...
			*/
			if (handle->buffer_head <= handle->buffer + handle->volume->no_of_bytes_per_serctor)
			{
				...
				ret = handle->volume->device->ReadSector(handle->volume->device->Media, sector_address, buff);
				if (ret != STORAGE_SUCCESS)
				{
					handle->busy = 0;
					return ret;
			...
			// write the cached sector to media
			*/
			ret = handle->volume->device->WriteSector(handle->volume->device->Media, sector_address, handle->buffer);
			if (ret != STORAGE_SUCCESS)
			{
				handle->busy = 0;
				return FAT_CANNOT_WRITE_MEDIA;
		...
		*/		
		...
		ret = handle->volume->device->ReadSector(handle->volume->device->Media, handle->directory_entry.sector_addr, buffer);
		if (ret != STORAGE_SUCCESS)
		{
			handle->busy = 0;
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
		...
		*/
		...
		// write the modified entry to the media
		*/			
		ret = handle->volume->device->WriteSector(handle->volume->device->Media, handle->directory_entry.sector_addr, buffer);
		if ( ret != STORAGE_SUCCESS )
		{
			handle->busy = 0;
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
		...
		*/
		{
			ret = handle->volume->device->ReadSector(handle->volume->device->Media, handle->directory_entry.sector_addr + 1, buffer);
			if (ret != STORAGE_SUCCESS)
			{
				handle->busy = 0;
				#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			...
			ret = handle->volume->device->WriteSector(handle->volume->device->Media, handle->directory_entry.sector_addr + 1, buffer);
			if ( ret != STORAGE_SUCCESS )
			{
				handle->busy = 0;
				#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
