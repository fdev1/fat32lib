...
uint16_t fat_mount_volume(FAT_VOLUME* volume, STORAGE_DEVICE* device) 
{
	...
	*/
	ret = volume->device->ReadSector(volume->device->Media, 0x0, buffer);
...
retry:
	...
	if (partitions_tried)
	{
		...
		*/
		if (partition_entry->lba_first_sector + 
			partition_entry->total_sectors > volume->device->GetTotalSectors(volume->device->Media))
		{
			partitions_tried++;
			goto retry;
		}
		...
		*/
		ret = volume->device->ReadSector(volume->device->Media, partition_entry->lba_first_sector, buffer);
		if (ret != STORAGE_SUCCESS)
		{
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			LEAVE_CRITICAL_SECTION(volume->sector_buffer_lock);
	...
	*/
	{
		...
		*/
		ret = volume->device->ReadSector(volume->device->Media, volume->no_of_reserved_sectors, buffer);
		if (ret != STORAGE_SUCCESS)
		{
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			LEAVE_CRITICAL_SECTION(volume->sector_buffer_lock);
	...
	*/
	if (volume->fs_type == FAT_FS_TYPE_FAT32)
	{
		...
		*/
		ret = volume->device->ReadSector(volume->device->Media, hidden_sectors + fsinfo_sector, buffer);
		if (ret != STORAGE_SUCCESS)
		{
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			LEAVE_CRITICAL_SECTION(volume->sector_buffer_lock);
	...
	*/
	...
	if (volume->fs_type == FAT_FS_TYPE_FAT32 && volume->fsinfo_sector != 0xFFFFFFFF)
	{
		...
		*/
		...
		// read the sector containing the FSInfo structure
		*/
		ret = volume->device->ReadSector(volume->device->Media, volume->fsinfo_sector, buffer);
		if (ret != STORAGE_SUCCESS)
		{
			#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
			LEAVE_CRITICAL_SECTION(volume->sector_buffer_lock);
		...
		*/
		/*if (fsinfo->LeadSig == 0x41615252 && fsinfo->StructSig == 0x61417272 && fsinfo->TrailSig == 0xAA550000)*/
		{
			...
			*/
			ret = volume->device->WriteSector(volume->device->Media, volume->fsinfo_sector, buffer);
			if (ret != STORAGE_SUCCESS)
			{
				#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
				LEAVE_CRITICAL_SECTION(volume->sector_buffer_lock);
...
) 
{
	...
	*/
	...
	// state buffer
	*/
	ret = volume->device->ReadSector(volume->device->Media, first_sector, query->buffer);
	if (ret != STORAGE_SUCCESS)
		return FAT_CANNOT_READ_MEDIA;
	/*
		...
		*/
		if (!first_entry)
		{
			...
			#endif
			{
				...
				*/
				ret = volume->device->ReadSector(volume->device->Media, sector_address, query->buffer);
				if (ret != STORAGE_SUCCESS)
					return FAT_CANNOT_READ_MEDIA;
				/*
				// set the 1st and current entry pointers
	...
	do 
	{
		...
		*/
		while (fat == 0 || sector < (first_sector_of_cluster + volume->no_of_sectors_per_cluster)) 
		{
			...
			*/
			ret = volume->device->ReadSector(volume->device->Media, sector, buffer);
			if (ret != STORAGE_SUCCESS)
			{
				#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
				LEAVE_CRITICAL_SECTION(volume->sector_buffer_lock);
				...
				*/
				if (IS_FREE_DIRECTORY_ENTRY(parent_entry)) 
				{
					...
					*parent_entry = new_entry->raw;
					...
					#endif
					if ((ret = volume->device->WriteSector(volume->device->Media, sector, buffer)) != STORAGE_SUCCESS)
					{
						#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
						LEAVE_CRITICAL_SECTION(volume->sector_buffer_lock);
						#elif defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_SHARED_BUFFER)
							...
							if (no_of_lfn_entries_found)
							{
								...
								*/
								...
								else
								{
									...
									*/
									ret = volume->device->WriteSector(volume->device->Media, sector, buffer);
									if (ret != STORAGE_SUCCESS)
									{
										#if defined(FAT_MULTI_THREADED) && defined(FAT_ALLOCATE_VOLUME_BUFFER)
										LEAVE_CRITICAL_SECTION(volume->sector_buffer_lock);
