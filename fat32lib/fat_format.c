...
) 
{
	...
	*/
	...
	for (i = 1; i < c; i++)
	{
		if (fs_type == FAT_FS_TYPE_FAT32)
		{
			...
			ret = device->WriteSector(device->Media, root_sector, buffer);
			if (ret != STORAGE_SUCCESS)
				return FAT_CANNOT_WRITE_MEDIA;
		}
		else
	...
	*/
	if (fs_type == FAT_FS_TYPE_FAT32)
	{
		ret = device->WriteSector(device->Media, ((root_cluster - 2) * no_of_sectors_per_cluster) + no_of_reserved_sectors + (no_of_fat_tables * fatsz) + root_dir_sectors, buffer);
		if (ret != STORAGE_SUCCESS)
			return FAT_CANNOT_WRITE_MEDIA;
	}
	else
