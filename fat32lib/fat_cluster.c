	...
	*/
	while (1) 
	{
		...
		*/
		...
		{
			ret = volume->device->ReadSector(volume->device->Media, entry_sector, buffer);
				...
				case FAT_FS_TYPE_FAT12:
				{
					...
					*/
					if (entry_offset == volume->no_of_bytes_per_serctor - 1)
					{
						...
						*/
						ret = volume->device->ReadSector(volume->device->Media, entry_sector + 1, buffer);
						if (ret != STORAGE_SUCCESS)
						{
							FAT_SET_LOADED_SECTOR(0xFFFFFFFF);
							FAT_UNLOCK_BUFFER();
						...
						*/
						if (last_fat_entry != FAT12_EOC)
						{
							if (last_entry_sector != entry_sector)
							{
								...
								*/
								ret = volume->device->ReadSector(volume->device->Media, last_entry_sector, buffer);
								if (ret != STORAGE_SUCCESS)
								{
									FAT_SET_LOADED_SECTOR(0xFFFFFFFF);
									FAT_UNLOCK_BUFFER();
							...
							*/
							if (last_entry_offset == volume->no_of_bytes_per_serctor - 1)
							{
								...
								*/
								...
								// load the next sector
								*/
								ret = volume->device->ReadSector(volume->device->Media, last_entry_sector + 1, buffer);
								if (ret != STORAGE_SUCCESS)
								{
									FAT_UNLOCK_BUFFER();
									FAT_RELINQUISH_WRITE_ACCESS();
	...
	*/
	...
	while (1)
	{
		...
		*/
		...
		{
			ret = volume->device->ReadSector(volume->device->Media, current_sector, buffer);
			if (ret != STORAGE_SUCCESS)
			{
				FAT_SET_LOADED_SECTOR(0xFFFFFFFF);
				FAT_UNLOCK_BUFFER();
	...
	FAT_VOLUME* volume, FAT_RAW_DIRECTORY_ENTRY* parent, uint32_t cluster, unsigned char* buffer) 
{
	...
	*/
	...
	ret = volume->device->WriteSector(volume->device->Media, current_sector++, buffer);
	if (ret != STORAGE_SUCCESS)
		return FAT_CANNOT_WRITE_MEDIA;	
	/*
...
static INLINE void fat_write_fat_sector(FAT_VOLUME* volume, uint32_t sector_address, unsigned char* buffer, uint16_t* ret)
{
	...
	*/
	*ret = volume->device->WriteSector(volume->device->Media, sector_address, buffer);
	if (*ret != STORAGE_SUCCESS)
		return;
	/*
	...
	*/
	...
	if (volume->no_of_fat_tables > 1)
	{
		...
		{
			*ret = volume->device->WriteSector(volume->device->Media, sector_address + (volume->fat_size * i), buffer);
			if (*ret != STORAGE_SUCCESS)
				return;
		}
	}
